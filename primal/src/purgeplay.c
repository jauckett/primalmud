...
if ( (player.char_specials_saved.idnum==12178) || 
     (player.char_specials_saved.idnum==11971))
 {
      ...
      if (okay && (player.level < LVL_CHAMP)) {
...
/* timeout=90; override */
	 ...
	 if ((time(0) - player.last_logon) > timeout) {
	    okay = 0;
	    sprintf(reason, "Level %2d idle for %4d days", player.level,
...
*/
      ...
      if (okay)
      {
         ...
         if (player.char_specials_saved.idnum>20000)
           player.char_specials_saved.idnum=new_id++;
if (!strcmp(player.name,"Murf") )
  player.char_specials_saved.idnum=2;
