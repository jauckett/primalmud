...
int trade_with(struct obj_data * item, int shop_nr)
{
  ...
  if(IS_OBJ_STAT(item,ITEM_NODROP)) return (OBJECT_NOTOK);
/* Darius - nosell */
		       ...
		       struct char_data * keeper, int shop_nr)
{
  ...
  extern void Crash_shopsave(struct char_data *ch,int shopnum);
...
void sort_keeper_objs(struct char_data * keeper, int shop_nr)
{
  ...
  while (list) {
	...
	!(get_obj_in_list_num(GET_OBJ_RNUM(temp), keeper->carrying,REAL))) {
		        ...
		        struct char_data * keeper, int shop_nr)
{
  ...
  struct obj_data *obj, *tag;
  int sellnum, sold = 0, goldamt = 0, price;
	int object_extracted = FALSE;
  ...
  /* added by hal to stop a money bug*/
  if ( sellnum > 1 )
  {
   sprintf(buf , "%s Try selling them one at a time please!" ,
           GET_NAME(ch) );
   do_tell (keeper, buf, cmd_tell, 0);
   return;
  }
  ...
  /* if (GET_GOLD(keeper) + SHOP_BANK(shop_nr) < sell_price(ch, obj, shop_nr))  */
	if (sell_price(ch, obj, shop_nr) > 200000)
	{
    /* sprintf(buf, shop_index[shop_nr].missing_cash1, GET_NAME(ch)); */
		price = number(200000, MIN(500000, sell_price(ch, obj, shop_nr)));
    sprintf(buf, "%s I can only afford to pay %d for that", GET_NAME(ch), price);
    do_tell(keeper, buf, cmd_tell, 0);
    /* return; */
  }
	else
...
/* JA if he dosn't already have it and it isn't an expensive item */
    if ((!already_has_obj(ch, obj, keeper->carrying)) && !price)
    {	
      if(IS_OBJ_STAT(obj,ITEM_NOSELL)) 
			{
				extract_obj(obj); 
				object_extracted = TRUE;
			}
      else 
...
/* JA if the selling price was above the limit */
		if (!price)
			price = sell_price(ch, obj, shop_nr);
    /* goldamt += sell_price(ch, obj, shop_nr); */
    goldamt += price;

		/* don't want shop keepers running out of cash */
...
/*  if(IS_OBJ_STAT(obj,ITEM_NOSELL)) */
	if (object_extracted)
		sprintf(buf,"The shopkeeper puts %s away.\n\r",tempstr);
  else 
		         ...
		         struct char_data * keeper, int shop_nr)
{
  ...
  sprintf(buf, "%s I'll give you %s%d%s gold coins for that!", GET_NAME(ch),
	  CCGOLD(ch,C_NRM),sell_price(ch, obj, shop_nr),CCNRM(ch,C_NRM));
  do_tell(keeper, buf, cmd_tell, 0);
...
int ok_damage_shopkeeper(struct char_data * ch, struct char_data * victim)
{
  ...
  if (IS_NPC(victim) && (mob_index[GET_MOB_RNUM(victim)].func == shop_keeper))
    for (index = 0; index < top_shop; index++)
      if ((GET_MOB_RNUM(victim) == SHOP_KEEPER(index)) && !SHOP_KILL_CHARS(index)&& ch!=NULL) 
			{
				if(!IS_NPC(ch)){
				   do_action(victim, GET_NAME(ch), cmd_slap, 0);
				   act("$n shouts 'Guards! Guards! Guards!'", FALSE, victim, 0, 0, TO_ROOM);
				   send_to_char("The guards arrive and beat you senseless!\n\r", ch);
