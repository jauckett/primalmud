		        ...
		        bool add)
{
  ...
  case APPLY_DEX:
    if (mod>0)
    {
      if (GET_REAL_DEX(ch)<22)
        if (GET_AFF_DEX(ch)+mod>21)
          GET_AFF_DEX(ch)=21;
        else
          GET_AFF_DEX(ch)+=mod;
    }
  ...
  case APPLY_INT:
    if (mod>0)
    {
      if(GET_AFF_INT(ch)+mod<=MAX(21,GET_REAL_INT(ch)))
        GET_AFF_INT(ch)+=mod;
      else GET_AFF_INT(ch)=MAX(21,GET_REAL_INT(ch));
    }
    else
      GET_AFF_INT(ch) += mod;
    break;
  case APPLY_WIS:
    if (mod>0)
    {
      if (GET_REAL_WIS(ch)<22)
        if (GET_AFF_WIS(ch)+mod>21)
          GET_AFF_WIS(ch)=21;
        else
          GET_AFF_WIS(ch)+=mod;
    }
 ...
 case APPLY_CON:
    if (mod>0)
    {
      if (GET_REAL_CON(ch)<22)
        if (GET_AFF_CON(ch)+mod>21)
          GET_AFF_CON(ch)=21;
        else
          GET_AFF_CON(ch)+=mod;
    }
  ...
  case APPLY_CHA:
    if (mod>0)
    {
      if (GET_REAL_CHA(ch)<22)
        if (GET_AFF_CHA(ch)+mod>21)
          GET_AFF_CHA(ch)=21;
        else
          GET_AFF_CHA(ch)+=mod;
    }
		      ...
		      bool add_dur, bool avg_dur, bool add_mod, bool avg_mod)
{
  ...
  for (hjp = ch->affected; !found && hjp; hjp = hjp->next) 
    ...
    if (hjp->type == af->type && hjp->location == af->location) 
		{
      if (add_dur)
				af->duration = MIN(af->duration+hjp->duration,MAX_DURATION);
      if (avg_dur)
				af->duration >>= 1;
      if (add_mod)
...
/*  assert(pos >= 0 && pos < NUM_WEARS); */
  ...
  if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch) && !IS_NPC(ch)) ||
      (IS_OBJ_STAT(obj, ITEM_ANTI_GOOD) && IS_GOOD(ch) && !IS_NPC(ch)) ||
...
struct obj_data *get_obj_in_list_num(int num, struct obj_data * list, int type)
{
  struct obj_data *i;
...
/* search all over the world for a char by name, and return pointer */
struct char_data *get_char(char *name)
{
  struct char_data *i;

  for (i = character_list; i; i = i->next)
    if (!strcmp(GET_NAME(i),name))
      return i;
...
void extract_obj(struct obj_data * obj)
{
  ...
  // If it's a mount, dismount the character
  if( OBJ_RIDDEN(obj) ) {
	send_to_char("Your mount disappears!\r\n", OBJ_RIDDEN(obj));
	act("$n's mount disappears.", FALSE, OBJ_RIDDEN(obj), 0, 0, TO_ROOM);
...
void extract_char(struct char_data * ch)
{
  ...
  //Artus> This should fix that hunt bug nicely.
  for (k = character_list; k; k = k->next)
  {
    if (!IS_NPC(k) && (HUNTING(k) == ch))
    {
      HUNTING(k) = NULL;
...
/* DM search all over the world for a player by id, and return pointer */
struct char_data *get_player_id(long idnum)
{
  struct char_data *i;
...
struct char_data *get_char_room_vis(struct char_data * ch, char *name)
{
  ...
  strcpy(tmp, name);
/*
  BM removed this CRAP allowed people to remotely look, social etc other players
  if (!(number = get_number(&tmp)))
...
struct char_data *get_char_vis(struct char_data * ch, char *name,int all_worlds)
{
  extern int get_world(struct char_data *ch);
  struct char_data *i;
  int j = 0, number;
  ...
  for (i = character_list; i && (j <= number); i = i->next)
    if (isname(tmp, i->player.name) && (all_worlds || (world[ch->in_room].zone == GOD_ROOMS_ZONE)
      || (get_world(ch) == get_world(i))) && CAN_SEE(ch, i))
      if (++j == number)
	return i;
		     ...
		     struct char_data ** tar_ch, struct obj_data ** tar_obj)
{
  ...
  *tar_obj = NULL;
  ...
  if (IS_SET(bitvector, FIND_CHAR_WORLD)) {
    if ((*tar_ch = get_char_vis(ch, name,FALSE))) {
