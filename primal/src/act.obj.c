...
************************************************************************ */
/*
  Modified by brett murphy to allow Level Restrictions on items
*/
...
void move_ship(void)
{
  ...
  switch (loc)
  {
    ...
    case 1 :              /* en-route to island */
      ...
      if ((obj = get_obj_in_list_num(8615, world[real_room(8690)].contents,VIRTUAL)))
      {     /* remove the ship */
        obj_from_room(obj);
        extract_obj(obj);
      }     /* remove the gangway */
      if ((obj = get_obj_in_list_num(8616, world[real_room(8669)].contents,VIRTUAL)))
      {
        obj_from_room(obj);
        extract_obj(obj);
      }
    ...
    case 3 :              /* en-route to haven */
      ...
      if ((obj = get_obj_in_list_num(8617, world[real_room(8854)].contents,VIRTUAL)))
      {
        obj_from_room(obj);
        extract_obj(obj);
      }
      if ((obj = get_obj_in_list_num(8618, world[real_room(8629)].contents,VIRTUAL)))
      {
        obj_from_room(obj);
        extract_obj(obj);
      }
...
void move_falcon(void)
{
  ...
  switch (loc)
  {
    ...
    case 1 :              /* en-route to island */
      ...
      if ((obj = get_obj_in_list_num(25015, world[real_room(25019)].contents,VIRTUAL)))
      {     /* remove the ship */
        obj_from_room(obj);
        extract_obj(obj);
      }     /* remove the gangway */
      if ((obj = get_obj_in_list_num(25016, world[real_room(25044)].contents,VIRTUAL)))
      {
        obj_from_room(obj);
        extract_obj(obj);
      }
    ...
    case 3 :              /* en-route to haven */
      ...
      send_to_room("\n\rThe Millenium Falcon takes off!", real_room(25044));
      if ((obj = get_obj_in_list_num(25015, world[real_room(22036)].contents,VIRTUAL)))
      {
        obj_from_room(obj);
        extract_obj(obj);
      }
      if ((obj = get_obj_in_list_num(25017, world[real_room(25044)].contents,VIRTUAL)))
      {
        obj_from_room(obj);
        extract_obj(obj);
      }
...
void get_check_money(struct char_data * ch, struct obj_data * obj)
{
  if ((GET_OBJ_TYPE(obj) == ITEM_MONEY) && (GET_OBJ_VAL(obj, 0) > 0)) {
    ...
    if (GET_OBJ_VAL(obj, 0) > 1) {
      sprintf(buf, "There were %s%d%s coins.\r\n", 
		CCGOLD(ch,C_NRM),GET_OBJ_VAL(obj, 0),CCNRM(ch,C_NRM));
      send_to_char(buf, ch);
    }
		     ...
		     byte mode, char *sname, sh_int RDR)
{
  ...
  if (IS_OBJ_STAT(obj, ITEM_NODROP)) 
	...
	/* JA if its a house then check objs in room first */
	if (mode == SCMD_DROP)
		if (find_house(world[ch->in_room].number) != -1)
			if (num_items_room(world[ch->in_room].contents) >= MAX_HOUSE_CONTENT)
			{
				send_to_char("The house is so cluttered with stuff you can't drop anymore.\r\n", ch);
				return(0);
			}
  ...
  if ((mode == SCMD_DONATE) && IS_OBJ_STAT(obj, ITEM_NODONATE))
    mode = SCMD_JUNK;
  if ((mode == SCMD_DONATE) && (GET_OBJ_LR(obj) > 5))
		            ...
		            int amount)
{
  ...
  send_to_char(OK, ch);
  sprintf(buf, "$n gives you %s%d%s gold coins.", 
...
ACMD(do_eat)
{
  ...
  if ((GET_OBJ_TYPE(food) != ITEM_FOOD) && (GET_LEVEL(ch) < LVL_IS_GOD)) {
  ...
  if (GET_OBJ_VAL(food, 3) && (GET_LEVEL(ch) < LVL_IS_GOD)) {
...
/* modifications made to perform_wear to allow magic eq - 25/12/95 Vader */
...
ACMD(do_wield)
{
  ...
  if (!*arg)
    ...
    {
      if ((!CAN_WEAR(obj, ITEM_WEAR_WIELD)) || (GET_OBJ_VAL(obj, 3) & 0x4000))
        send_to_char("You can't wield that.\r\n", ch);
      else 
        if (GET_OBJ_WEIGHT(obj) > str_app[STRENGTH_AFF_APPLY_INDEX(ch)].wield_w)
          send_to_char("It's too heavy for you to use.\r\n", ch);
        else
        {
     //     /* JA check if this is LR based on average dam */
     //     max_damage = GET_OBJ_VAL(obj, 1) * GET_OBJ_VAL(obj, 2);
     //     if ((max_damage > (GET_LEVEL(ch)+5)) && (max_damage > 20) 
     //          && (GET_LEVEL(ch) < LVL_ANGEL))
     //       /* can't wield it */
     //       send_to_char("You can't seem to figure out how to wield this.\n\r", ch); 
     //     else

     // DM, 11/99 - changed to obj level
          if (!invalid_level(ch,obj))
...
/* modifications made to perform_remove to remove magic eq affs - Vader */
