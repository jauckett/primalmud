...
#define C_MOB_SPEC(zone,mob)					\
	if ((check = real_mobile(((zone)*100)+(mob))) < 0) {	\
	  if (!mini_mud)					\
	    basic_mud_log("SYSERR: assign_kings_castle(): can't find mob #%d.", ((zone)*100)+(mob));	\
	} else							\
...
int do_npc_rescue(struct char_data * ch_hero, struct char_data * ch_victim)
{
  ...
  set_fighting(ch_hero, ch_bad_guy, SKILL_RESCUE);
  set_fighting(ch_bad_guy, ch_hero, SKILL_RESCUE);
  return (TRUE);
}
...
int castle_cleaner(struct char_data *ch, int cmd, int gripe)
{
  ...
  for (i = world[ch->in_room].contents; i; i = i->next_content) {
    ...
    obj_from_room(i);
    obj_to_char(i, ch, __FILE__, __LINE__);
    return (TRUE);
  }
