...
send_clan_format (struct char_data *ch)
{
  ...
  if ((r >= clan[c].privilege[CP_SET_APPLEV])
      ...
      || (r >= clan[c].privilege[CP_SET_DESC]))
...
void
do_clan_reset (struct char_data *ch, char *arg)
{
  int c, i, j;
  ...
  if ((real = real_mobile (c + (CLAN_ZONE * 100))) > -1)
    {
      ...
      GET_DDESC (guard) =
	str_dup ("This clan guard has not been described yet.\r\n");
      GET_HIT (guard) = GET_MANA (guard) = GET_NDD (guard) = GET_SDD (guard) =
	1;
    ...
    {
      sprintf (buf, "Clan guard [%d] not found while resetting!",
	       (c + (CLAN_ZONE * 100)));
      basic_mud_log (buf);
    }
  if ((real = real_mobile (c + (CLAN_ZONE * 100) + MAX_CLANS)) > -1)
    {
      ...
      GET_DDESC (healer) =
	str_dup ("This clan healer has not been described yet.\r\n");
      GET_HIT (healer) = GET_MANA (healer) = GET_NDD (healer) =
	GET_SDD (healer) = 1;
    ...
    {
      sprintf (buf, "Clan healer [%d] not found while resetting!",
	       (c + (CLAN_ZONE * 100)));
      basic_mud_log (buf);
    }
  ...
  if ((real = real_room (c + (CLAN_ZONE * 100))) != NOWHERE)
    {
      ...
      room->description =
	strdup ("This guard post is yet to be described.\r\n");
      REMOVE_BIT (room->room_flags, ROOM_REGEN_2);
    }
  else
    {
      sprintf (buf, "Clan room [%d] not found while resetting!",
	       (c + (CLAN_ZONE * 100)));
      basic_mud_log (buf);
    }
  ...
  if ((real = real_room (c + MAX_CLANS + (CLAN_ZONE * 100))) != NOWHERE)
	...
	if (!(((c % 2 == 1) && (i == EAST)) || ((c % 2 == 0) && (i == WEST))))
      ...
      free (room->description);
      room->description =
	strdup ("This clan hall is yet to be described.\r\n");
      REMOVE_BIT (room->room_flags, ROOM_REGEN_2);
    }
  else
    {
      sprintf (buf, "Clan room [%d] not found while resetting!",
	       (c + MAX_CLANS + (CLAN_ZONE * 100)));
      basic_mud_log (buf);
    }
  for (i = 2; i <= CLAN_ROOM_MAX; i++)
    {				/* Rooms */
      if ((real =
	   real_room (c + (CLAN_ZONE * 100) + (i * MAX_CLANS))) != NOWHERE)
	{
	  room = &world[real];
	  free (room->name);
	  ...
	  free (room->description);
	  room->description =
	    strdup ("This clan room is yet to be described.\r\n");
	  REMOVE_BIT (room->room_flags, ROOM_REGEN_2);
	}
      else
	{
	  sprintf (buf, "Clan room [%d] not found while resetting!",
		   (c + (CLAN_ZONE * 100) + (i * MAX_CLANS)));
	  basic_mud_log (buf);
	}
    }
...
void
clan_rel_change (int us, int them, int amt)
{
  if ((us < 0) || (them < 0) || (us > num_of_clans) || (them > num_of_clans)
      || (us == them) || (amt == 0))
    return;
  if (amt > 0)
    GET_CLAN_REL (us, them) =
      MAX (GET_CLAN_REL (us, them) + amt, CLAN_REL_MIN);
  else
    GET_CLAN_REL (us, them) =
      MIN (GET_CLAN_REL (us, them) + amt, CLAN_REL_MAX);
  GET_CLAN_REL (them, us) = GET_CLAN_REL (us, them);
}

void
clan_rel_inc (struct char_data *ch, struct char_data *vict, int amt)
{
  int us, them, amount;
  amount = amt;
...
int
clan_check_priv (struct char_data *ch, int priv)
{
  int clan_num;
  if (!LR_FAIL (ch, LVL_CLAN_GOD))
    return CPC_IMM;
...
/* Clan Exp Gaining... */
void
gain_clan_exp (struct char_data *ch, long amount)
{
  int c = 0;
  char cin[80];
  ...
  while (clan[c].exp > clan_levels[clan[c].level])
    {				/* Clan Levels */
      ...
      clan[c].level++;
      sprintf (cin, "%s has advanced to level %d.", clan[c].name,
	       clan[c].level);
...
do_clan_create (struct char_data *ch, char *arg)
{
  ...
  if (GET_LEVEL (leader) < LVL_CLAN_MIN)
    {
      send_to_char
	("You cannot set someone that inexperienced as the clan leader.\r\n",
	 ch);
      return;
    }
  ...
  sprintf (cin, "A new clan, %s has been formed.",
...
void
do_clan_score (struct char_data *ch)
{
  int i, j;
...
/* Clan charging */
int
do_clan_charge (struct char_data *ch, int amount)
{
  if (!LR_FAIL (ch, LVL_CLAN_GOD))
    return 1;
...
do_clan_destroy (struct char_data *ch, char *arg)
{
  ...
  for (j = 0; j <= top_of_p_table; j++)
    {
      if ((victim =
	   get_player_online (ch, (player_table + j)->name, FIND_CHAR_INVIS)))
	{
...
void
do_clan_stat (struct char_data *ch, char *arg)
{
  int c, i, crap = 0;
  ...
  sprintf (buf, "%sMembers: [%d] Power: [%d] Bank: [%ld00] Ranks: [%d]\r\n",
	   buf, clan[c].members, clan[c].power, clan[c].treasure,
	   clan[c].ranks);
  ...
  sprintf (buf,
	   "%sTaxrate: [%d] Applev: [%d] Spells: [%d/%d/%d/%d/%d]\r\nRelations: ",
	   buf, clan[c].taxrate, clan[c].app_level, clan[c].spells[0],
	   clan[c].spells[1], clan[c].spells[2], clan[c].spells[3],
	   clan[c].spells[4]);

  /* Line 5: Relations */
  for (i = 0; i < num_of_clans; i++)
    sprintf (buf, "%s%d(%d)%s", buf, clan[i].id, GET_CLAN_REL (c, i),
	     ((i < (num_of_clans - 1)) ? " " : "\r\nFrags: "));
  ...
  /* Line 8: Privileges */
  for (i = 0; i < NUM_CP; i++)
    sprintf (buf, "%s%s[%d]%s", buf, clan_privileges[i], clan[c].privilege[i],
	     ((i < (NUM_CP - 1)) ? ", " : "\r\nRanks: "));

  /* Line 9: Rank Titles */
  for (i = 0; i < clan[c].ranks; i++)
    sprintf (buf, "%s%d-%s%s", buf, (i + 1), clan[c].rank_name[i],
	     ((i < (clan[c].ranks - 1)) ? ", " : "\r\n"));
...
void
do_clan_recruit (struct char_data *ch, char *arg)
{
...
struct char_data *
get_player (struct char_data *ch, char *name)
{
  struct char_data *i;
...
void
do_clan_banish (struct char_data *ch, char *arg)
{
  extern int top_of_p_table;
  ...
  if ((vict = get_player (ch, arg)) != NULL)
	...
	{
	  send_to_char
	    ("You cannot banish someone who is of equal or higher rank than yourself.\r\n",
	     ch);
	  return;
	}
  ...
  if (immcom)
    {
      send_to_char
	("Couldn't find that player online.. Perhaps you need to set file 'x' clan 0.\r\n",
	 ch);
      return;
    }
  ...
  for (i = 0; i <= top_of_p_table; i++)
	...
	{
	  if ((!strcasecmp (tch.name, arg))
	      && (tch.player_specials_saved.clan == clan[clan_num].id))
	    {
	      if (tch.player_specials_saved.clan_rank >= GET_CLAN_RANK (ch))
		{
		  send_to_char
		    ("Cannot banish someone who's not ranked lower than yourself.\r\n",
		     ch);
		  return;
		}
	      i = top_of_p_table;
  ...
  send_to_char
    ("Could not find anyone by that name belonging to your clan.\r\n", ch);
  return;
}
...
do_clan_demote (struct char_data *ch, char *arg)
{
  ...
  sprintf (buf, "%s has demoted you to %s.\r\n",
	   ((CAN_SEE (ch, vict)) ? ch->player.name : "someone"),
	   clan[clan_num].rank_name[GET_CLAN_RANK (vict) - 1]);
  send_to_char (buf, vict);
  send_to_char ("Done.\r\n", ch);
  sprintf (cin, "%s has been demoted to %s.", GET_NAME (vict),
	   clan[clan_num].rank_name[GET_CLAN_RANK (vict) - 1]);
  do_clan_infocom (GET_CLAN (vict), 0, cin);
  return;
}
...
do_clan_promote (struct char_data *ch, char *arg)
{
  ...
  send_to_char (buf, vict);
  send_to_char ("Done.\r\n", ch);
  sprintf (cin, "%s promoted to %s.", GET_NAME (vict),
	   clan[clan_num].rank_name[GET_CLAN_RANK (vict) - 1]);
  do_clan_infocom (GET_CLAN (vict), 0, cin);
  return;
}
...
void
do_clan_who (struct char_data *ch, char *arg)
{
  extern int top_of_p_table;
  ...
  for (i = 0; (int) i <= top_of_p_table; i++)
	...
	{
	  if ((tch.player_specials_saved.clan == test_clan)
	      && (tch.player_specials_saved.clan_rank > 0))
	    {
	      sprintf (line_disp, "%-15s  %s\r\n", tch.name,
		       clan[clan_num].rank_name[tch.player_specials_saved.
						clan_rank - 1]);
...
void
do_clan_about (struct char_data *ch, char *arg)
{
  int clan_num;
...
/* Clan Info */
void
do_clan_infocom (int clan_one, int clan_two, char *arg)
{
...
do_clan_apply (struct char_data *ch, char *arg)
{
  ...
  if (!(*arg))
    {
      send_to_char
	("To which clan were you wanting to apply? (Try \"clan list\")\r\n",
	 ch);
      return;
    }
  if ((clan_num = find_clan_by_sid (arg)) < 0)
...
do_clan_info (struct char_data *ch, char *arg)
{
  ...
  if (!(*arg))
    {
      ...
      for (i = 0; i < num_of_clans; i++)
	sprintf (buf,
		 "%s[%-2d]  %-20s  Level: %2d  Appfee: %9d  Tax Rate: %d%%\r\n",
		 buf, clan[i].id, clan[i].name, clan[i].level,
  ...
  for (j = 0; j < clan[i].ranks; j++)
    sprintf (buf, "%s%s%s ", buf, clan[i].rank_name[j],
	     ((j == clan[i].ranks - 1) ? "." : ","));
  sprintf (buf, "%s\r\nMembers    : %d\r\nPower      : %d\r\n", buf,
  ...
  for (j = 0; j < NUM_CP; j++)
    sprintf (buf, "%s   %-10s: %s%s", buf, clan_privileges[j],
	     clan[i].rank_name[clan[i].privilege[j] - 1],
	     (((j % 2) == 0) ? "     " : "\r\n"));
...
void
do_clan_changefees (struct char_data *ch, char *arg, int action)
{
  ...
  if (!(*arg))
	...
	case CM_DUES:
	  if (LR_FAIL (ch, LVL_CLAN_GOD))
	    send_to_char
	      ("Syntax: clan set tax <2-50>  (This number is a percentage.\r\n",
	       ch);
	  else
	    send_to_char
	      ("Syntax: clan set tax <clan num> <2-50>  (Percentage)\r\n",
	       ch);
	  break;
	default:
  ...
  switch (action)
	...
	{
	  sprintf (buf,
		   "Application fee must be between %d and %d coins.\r\n",
...
void
do_clan_set_ranks (struct char_data *ch, char *arg)
{
  int i, j;
  int clan_num, immcom = 0;
  ...
  for (j = 0; j <= top_of_p_table; j++)
      ...
      else
	{
	  ...
	  if (chdata.player_specials_saved.clan == clan[clan_num].id)
	    {
	      if (chdata.player_specials_saved.clan_rank <
		  clan[clan_num].ranks
		  && chdata.player_specials_saved.clan_rank > 0)
  ...
  sprintf (buf, "Number of ranks is now %d. (%d coins deducted)\r\n",
	   new_ranks, (immcom) ? 0 : amtreq);
  send_to_char (buf, ch);
  strcpy (clan[clan_num].rank_name[new_ranks - 1],
	  clan[clan_num].rank_name[clan[clan_num].ranks - 1]);
  if (new_ranks > clan[clan_num].ranks)
    {
  ...
  for (i = 0; i < NUM_CP; i++)
    if (clan[clan_num].privilege[i] == clan[clan_num].ranks)
      ...
      clan[clan_num].privilege[i] =
	MIN (clan[clan_num].privilege[i], new_ranks);
...
do_clan_sp (struct char_data *ch, char *arg, int priv)
{
  ...
  if (!(*arg))
	...
	send_to_char
	  ("Syntax: clan set privilege <privilege> <clan #> <rank #>\r\n",
	   ch);
      return;
    }
...
void
do_clan_set_desc (struct char_data *ch, char *arg)
{
  int clan_num;
...
/* Clan Privileges */
void
do_clan_set_priv (struct char_data *ch, char *arg)
{
  ...
  if (is_abbrev (arg1, "setabout"))
    {
      do_clan_sp (ch, arg2, CP_SET_DESC);
      return;
    }
  if (is_abbrev (arg1, "recruit"))
    {
      do_clan_sp (ch, arg2, CP_RECRUIT);
      return;
    }
  if (is_abbrev (arg1, "banish"))
    {
      do_clan_sp (ch, arg2, CP_BANISH);
      return;
    }
  ...
  if (is_abbrev (arg1, "enhance"))
    {
      do_clan_sp (ch, arg2, CP_ENHANCE);
      return;
    }
  if (is_abbrev (arg1, "room"))
    {
      do_clan_sp (ch, arg2, CP_ROOM);
      return;
    }
  if (is_abbrev (arg1, "setdesc"))
    {
      do_clan_sp (ch, arg2, CP_SET_DESC);
      return;
    }
  if (is_abbrev (arg1, "enable"))
    {
      do_clan_sp (ch, arg2, CP_ENABLE);
      return;
    }
...
/* Clan Room Stuff */
void
do_clan_room (struct char_data *ch, char *arg)
{
  char arg1[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH];
  int clan_num = 0;
  zone_rnum real_zone (zone_vnum vnum);
  ...
  if ((GET_ROOM_VNUM ((ch)->in_room) < (CLAN_ZONE * 100))
      || (GET_ROOM_VNUM ((ch)->in_room) > ((CLAN_ZONE * 100) + 351)))
    {
  ...
  if (((GET_ROOM_VNUM ((ch)->in_room) - (CLAN_ZONE * 100)) % MAX_CLANS) !=
      clan_num)
    {
  ...
  if (is_abbrev (arg1, "title"))
	...
	{
	  send_to_char
	    ("New title is too long, please keep to under 75 chars.\r\n", ch);
	  return;
	}
      if (do_clan_charge (ch, CLC_ROOMTD) < 1)
	{
	  sprintf (buf,
		   "You'll need %d gold coins to change the room title.\r\n",
		   CLC_ROOMTD);
	  send_to_char (buf, ch);
	  return;
	}
  ...
  if (is_abbrev (arg1, "regen"))
	...
	{
	  sprintf (buf,
		   "Current room is now Regen*2. (%d coins deducted).\r\n",
		   CLC_REGENROOM);
	  send_to_char (buf, ch);
	  return;
	}
  ...
  if (is_abbrev (arg1, "description"))
	...
	{
	  sprintf (buf,
		   "Changing a room's description costs %d coins, which you do not have.\r\n",
		   CLC_ROOMTD);
	  send_to_char (buf, ch);
	  return;
	}
  ...
  if (is_abbrev (arg1, "build"))
      ...
      for (i = 1; i < CLAN_ROOM_MAX; i++)
	{
	  ...
	  for (j = 0; j < NUM_OF_DIRS; j++)
	    if (W_EXIT
		(real_room ((i * MAX_CLANS) + clan_num + (CLAN_ZONE * 100)),
		 j))
	      found = 0;
	...
	{
	  sprintf (buf,
		   "Hiring builders for your new room will cost %d coins.\r\n",
		   CLC_BUILD);
	  send_to_char (buf, ch);
	  return;
	}
      CREATE (world[ch->in_room].dir_option[direction],
	      struct room_direction_data, 1);
      CREATE (world[new_room].dir_option[rev_dir[direction]],
	      struct room_direction_data, 1);
...
void
do_clan_name_mob (struct char_data *ch, char *arg)
{
  char arg1[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH];
  int clan_num = 0;
...
void
do_clan_describe_mob (struct char_data *ch, char *arg)
{
  char arg1[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH];
  int clan_num = 0;
...
void
do_clan_showrel (struct char_data *ch, char *arg)
{
  int i, n, c;
...
/* Clan Enable Format */
void
send_clan_enable_f (struct char_data *ch)
{
  int c, r;
  c = find_clan_by_id (GET_CLAN (ch));
  r = GET_CLAN_RANK (ch);
  ...
  sprintf (buf, "%shall      - Clan Hall%s\r\n", buf,
	   (CLAN_HASOPT (c, CO_HALL) ? " (enabled)" : ""));
  sprintf (buf, "%shealer    - Clan Healer%s\r\n", buf,
	   (CLAN_HASOPT (c, CO_HEALER) ? " (enabled)" : ""));
  sprintf (buf, "%sboard     - Clan Board%s\r\n", buf,
	   (CLAN_HASOPT (c, CO_BOARD) ? " (enabled)" : ""));
  sprintf (buf, "%sctalk     - Clan && Clan Leader Channels.%s\r\n", buf,
	   (CLAN_HASOPT (c, CO_TALK) ? " (enabled)" : ""));
  send_to_char (buf, ch);
}

/* Clan Enable */
void
do_clan_enable (struct char_data *ch, char *arg)
{
  ...
  if (is_abbrev (arg, "ctalk"))
      ...
      if (do_clan_charge (ch, CLC_ENTALK) > 0)
	{
      ...
      sprintf (buf,
	       "You will need %d coins to activate the clantalk channel.\r\n",
	       CLC_ENTALK);
      send_to_char (buf, ch);
      return;
    }
...
void
do_clan_enhance (struct char_data *ch, char *arg)
{
  char arg1[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH];
...
do_clan_set (struct char_data *ch, char *arg)
{
  ...
  if (is_abbrev (arg1, "about"))
    {
      do_clan_set_desc (ch, arg2);
      return;
    }
  if (is_abbrev (arg1, "ranks"))
    {
      do_clan_set_ranks (ch, arg2);
      return;
    }
  if (is_abbrev (arg1, "title"))
    {
  ...
  if (is_abbrev (arg1, "privilege"))
    {
      do_clan_set_priv (ch, arg2);
      return;
    }
  if (is_abbrev (arg1, "tax"))
    {
      do_clan_changefees (ch, arg2, CM_DUES);
      return;
    }
  if (is_abbrev (arg1, "appfee"))
    {
      do_clan_changefees (ch, arg2, CM_APPFEE);
      return;
    }
...
ACMD (do_clan)
{
  ...
  if (is_abbrev (arg1, "recruit"))
    {
      do_clan_recruit (ch, arg2);
      return;
    }
  if (is_abbrev (arg1, "banish"))
    {
      do_clan_banish (ch, arg2);
      return;
    }
  ...
  if (is_abbrev (arg1, "about"))
    {
      do_clan_about (ch, arg2);
      return;
    }
  if (is_abbrev (arg1, "enhance"))
    {
      do_clan_enhance (ch, arg2);
      return;
    }
  if (is_abbrev (arg1, "name"))
    {
      do_clan_name_mob (ch, arg2);
      return;
    }
  if (is_abbrev (arg1, "describe"))
    {
      do_clan_describe_mob (ch, arg2);
      return;
    }
  if (is_abbrev (arg1, "room"))
    {
      do_clan_room (ch, arg2);
      return;
    }
  if (is_abbrev (arg1, "withdraw"))
    {
  ...
  if (is_abbrev (arg1, "balance"))
    {
      do_clan_bank (ch, arg2, CB_BALANCE);
      return;
    }
  if (is_abbrev (arg1, "relations"))
    {
      do_clan_showrel (ch, arg2);
      return;
    }
  ...
  if (is_abbrev (arg1, "enable"))
    {
      do_clan_enable (ch, arg2);
      return;
    }
  if (is_abbrev (arg1, "reset"))
    {
      do_clan_reset (ch, arg2);
      return;
    }
  send_clan_format (ch);
}
...
ACMD (do_pkill)
{
  ...
  if ((!(*argument)) || (strcasecmp ("enable", argument)))
    {
      send_to_char
	("Becoming pkill is irreversable, until you join a clan.\r\n"
...
int
do_clan_heal (struct char_data *ch, struct char_data *vict)
{
  ...
  switch GET_LEVEL
	...
	{
	  GET_MOVE (vict) =
	    MAX ((GET_MAX_MOVE (vict) >> 1) + 1, GET_MOVE (vict));
	  GET_MANA (vict) =
	    MAX ((GET_MAX_MANA (vict) >> 1) + 1, GET_MANA (vict));
	  GET_HIT (vict) =
	    MAX ((GET_MAX_HIT (vict) >> 1) + 1, GET_HIT (vict));
	  return TRUE;
	}
      add_h = 400 + number (0, 75);
      add_v = 75 + number (0, 25);
    ...
    default:
      send_to_char
	("The clan healer is too busy contemplating the meaning of life to heal you.\r\n",
	 vict);
...
void
clan_tax_update (struct char_data *ch, struct char_data *vict)
{
...
int
clan_can_hear (struct descriptor_data *d, int c)
{
  ...
  if (c < 0)
    {
	  ...
	  clan[find_clan_by_id (GET_CLAN (d->character))].ranks)
	return FALSE;
    }
...
int
pun_aggro_check (struct char_data *ch)
{
  struct char_data *i;
  int found = 0;
  ...
  for (i = world[ch->in_room].people; i; i = i->next_in_room)
    {
      if (!IS_NPC (i))
	continue;
      if ((STATE (ch->desc) == CON_PLAYING) && (i->in_room == ch->in_room)
	  && (FIGHTING (i) == NULL) && (CAN_SEE (i, ch)) && (AWAKE (i))
	  && (mob_index[GET_MOB_RNUM (i)].func != shop_keeper)
	  && !MOB_FLAGGED (i, MOB_NOKILL))
	{
	  act ("$n senses dishonour in you, and attacks hastily.", FALSE, i,
	       0, ch, TO_VICT);
...
void
punish_update (struct char_data *ch)
{
  int i;
...
void
perform_punish (struct char_data *ch, int ptype, int hours, int offence)
{
  if (IS_NPC (ch))		/* Mobs can fuck themselves :o) */
    return;

  if (hours == 0)
    {				/* Remove Punishment on 0 hours */
  ...
  if ((offence >= 0) && (hours != 0))
    {
      ...
      if (HAS_OFFENDED (ch, offence) < 1)
	HAS_OFFENDED (ch, offence) = 1;
    }

  if (hours < 0)
    {				/* Unlimited on <0 hours */
...
void
clan_pk_update (struct char_data *winner, struct char_data *loser)
{
  ...
  if ((INSTIGATOR (winner) == TRUE) && (GET_LEVEL (winner) < LVL_ANGEL))
      ...
      if (severity > 1)
	{
	  ...
	  sprintf (buf,
		   "Killing someone that small is weak. You are punished for the next %d hours.\r\n",
		   severity);
	  send_to_char (buf, winner);
	  perform_punish (winner, level_diff,
			  (PUN_HOURS (winner, level_diff) + severity),
			  OFF_PKILL);
	}
    }
...
SPECIAL (clan_healer)
{
  ...
  for (to = world[self->in_room].people; to; to = to->next_in_room)
	...
	send_to_char
	  ("The clan healer touches you. You feel mildly healthier.\r\n", to);
...
SPECIAL (clan_guard)
{
  ...
  if (LR_FAIL (ch, LVL_CLAN_GOD))
	...
	{
	  act ("$n slaps you, 'Thou art not a part of this clan. Begone!'",
	...
	{
	  act ("$n &rtells you, 'This clan's hall has not been built yet.'&n",
	       FALSE, self, 0, ch, TO_VICT);
	  return (TRUE);
	}
	...
	{			/* Only Allow Clan members... */
	  if ((us != them) && !CLAN_AT_PEACE (us, them))
	    {
	      act
		("$n slaps you, 'Thou art not a part of this clan. Begone!'",
		 FALSE, self, 0, ch, TO_VICT);
	      act ("$N slaps $n.", FALSE, ch, 0, self, TO_ROOM);
	      return (TRUE);
	    }
	    ...
	    {
	      act
		("$n &rtells you, 'Thy application must be approved before entering.'&n",
		 FALSE, self, 0, ch, TO_VICT);
	      return (TRUE);
	    }
	  act
	    ("$n waves a hand over you, and you find yourself in the clan room.",
	     FALSE, self, 0, ch, TO_VICT);
	  act ("$n has been transferred away to $s calling.", FALSE, ch, 0, 0,
	       TO_ROOM);
	}
      else if (CMD_IS ("bribe"))
	{			/* There is nothing money cannot buy. */
	  ...
	  if ((us != them) && !CLAN_AT_PEACE (us, them))
		...
		{
		  send_to_char
		    ("How much did you want to bribe the guard??\r\n", ch);
		  return (TRUE);
		}
		...
		case 110:
		  ...
		  break;
		case 100:
		  amt_req = 10000 * GET_LEVEL (ch);
		  break;
		case 90:
		  amt_req = 5000 * GET_LEVEL (ch);
		  break;
		case 80:
		  amt_req = 2500 * GET_LEVEL (ch);
		  break;
		case 70:
		  amt_req = 1000 * GET_LEVEL (ch);
		  break;
		case 60:
		  amt_req = 100 * GET_LEVEL (ch);
		  break;
		case 50:
		  amt_req = 0;
		  break;
		...
		amt_req -= (int) (amt_req / 3);
	      if ((amount < amt_req)
		  || (GET_CLAN_REL (us, them) < CLAN_REL_BLOOD_WAR))
		{
		  act ("$n takes the gold and laughs in your face.", FALSE,
		       self, 0, ch, TO_VICT);
		  act ("$N laughs hard at $n.", FALSE, ch, 0, self, TO_ROOM);
		...
		if (number (0, 5) > 0)
		  {		/* Anything higher and the guard will cum */
		    act ("$n takes the gold and laughs in your face.", FALSE,
			 self, 0, ch, TO_VICT);
		    act ("$N laughs hard at $n.", FALSE, ch, 0, self,
			 TO_ROOM);
		    return (TRUE);
		  }
	      act ("$n quickly pockets the coin and sneaks you inside.",
		   FALSE, self, 0, ch, TO_VICT);
	      act ("$N whistles dixie as $n disappears.", FALSE, ch, 0, self,
		   TO_ROOM);
	    }
	  else
	    {
	      act
		("$n looks at the gold, 'Don't worry about it, this one's on me!'",
		 FALSE, self, 0, ch, TO_VICT);
...
ACMD (do_sentence)
{
  ...
  for (i = 0; i < NUM_PUNISHES; i++)
	...
	sprintf (buf, "%s  (%3dhr): %s\r\n", buf,
		 ch->player_specials->saved.phours[i], punish_types[i]);
      else
  ...
  if (found == 0)
    {
      send_to_char
	("You have not been sentenced, you must have been well-behaved.\r\n",
	 ch);
      return;
    }
  send_to_char (buf, ch);
...
ACMD (do_punish)
{
  ...
  char syntax[] =
    "Syntax: punish <victim> <punishment> <hours> <offence> <reason>\r\n";
  ...
  if (!*argument)
    {
      send_to_char
	("Number of hours not specified. (0 = off, -1 = Permanent).\r\n", ch);
      return;
    }
  ...
  if (atoi (hours) > 0)
    sprintf (buf,
	     "(GC) &7%s&n has punished &7%s&n for &1%s&n. (&1%d&nhrs of &1%s&n).",
    ...
    sprintf (buf,
	     "(GC) &7%s&n has punished &7%s&n for &1%s&n. (Indefinite &1%s&n).",
	     GET_NAME (ch), GET_NAME (vict),
  ...
  if (atoi (hours) != 0)
    sprintf (buf,
	     "&RYou have been punished for &1%s&r. Type \"&gsentence&R\" for details.\r\n",
...
ACMD (do_ctalk)
{
  ...
  if (!*argument)
    {
      send_to_char
	("Perhaps you should think about what you want to say first..\r\n",
	 ch);
      return;
    }
	  ...
	  if (i->character->player_specials->saved.clan_rank >= minlev)
	    {
	      sprintf (buf, "%s clan talks%s, '%s'\r\n",
		       ((!CAN_SEE (i->character, ch)) ? "someone" : ch->
			player.name), level_string, argument);
	      send_to_char (buf, i->character);
	    }
	  else if (!LR_FAIL (ch, LVL_CLAN_GOD))
	    {
	      sprintf (buf, "%s clan talks [%d]%s, '%s'\r\n",
		       ((!CAN_SEE (i->character, ch)) ? "someone" : ch->
			player.name), GET_CLAN (ch), level_string, argument);
	      send_to_char (buf, i->character);
	    }
	}
    }
  ...
  for (i = descriptor_list; i; i = i->next)
      ...
      if ((i != ch->desc) && (clan_can_hear (i, -1)))
	{
	  sprintf (buf, "%s leader talks, '%s'\r\n",
		   ((!CAN_SEE (i->character, ch)) ? "someone" : ch->player.
		    name), argument);
	  send_to_char (buf, i->character);
	}
    }
