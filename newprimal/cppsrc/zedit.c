...
void zedit_sect_all(struct char_data *ch, int new_sect)
{
  ...
  if ((new_sect < 0) || (new_sect > NUM_ROOM_SECTORS_BASE))
  {
    sprintf(buf, "Syntax: zedit sect <sector type>\r\n\r\n"
	         "Valid sector types are:\r\n");
    for (i = 0; i <= NUM_ROOM_SECTORS_BASE; i++)
      sprintf(buf, "%s%2d - %-20.20s %s", buf, i, 
	      sector_types[SECT_TYPE_BASE][i], (i % 3 == 2) ? "\r\n" : "");
    if (!(i%2))
      strcat(buf, "\r\n");
    send_to_char(buf, ch);
    return;
  ...
  for (i = 0; i <= top_of_world; i++)
    if (world[i].zone == world[IN_ROOM(ch)].zone)
    {
      world[i].sector_type -= BASE_SECT(world[i].sector_type);
      world[i].sector_type += new_sect;
      modded++;
    }
  sprintf(buf, "%d rooms are now sectored as '%s'.\r\n", modded,
          sector_types[SECT_TYPE_BASE][new_sect]);
  send_to_char(buf, ch);
}
...
void zedit_setup(struct descriptor_data *d, int room_num)
{
  ...
  zone->world = zone_table[OLC_ZNUM(d)].world;
  zone->lifespan = zone_table[OLC_ZNUM(d)].lifespan;
  zone->zflag = zone_table[OLC_ZNUM(d)].zflag;
...
void zedit_new_zone(struct char_data *ch, int vzone_num)
{
  ...
  for (dsc = descriptor_list; dsc; dsc = dsc->next) {
    switch (STATE(dsc)) {
      case CON_REDIT:
        // DM - HERE
        basic_mud_log("OLC_ROOM(dsc)->zone (%d) += OLC_ZNUM(dsc) (%d)",
            OLC_ROOM(dsc)->zone, OLC_ZNUM(dsc)); 
...
void zedit_save_internally(struct descriptor_data *d)
{
  ...
  for (subcmd = 0; MYCMD.command != 'S'; subcmd++) {
    ...
    switch (MYCMD.command) {
      ...
      case 'E':
        if (mobloaded)
	{
	  objloaded = TRUE; // Artus> Should fix issue with give container/put.
          break;
	}
  ...
  if (OLC_ZONE(d)->number) {
    ...
    zone_table[OLC_ZNUM(d)].world = OLC_ZONE(d)->world;
    zone_table[OLC_ZNUM(d)].zflag = OLC_ZONE(d)->zflag;
...
void zedit_disp_menu(struct descriptor_data *d)
{
  ...
  sprintf(buf,
	  ...
	  grn, nrm, yel, OLC_ZONE(d)->name ? OLC_ZONE(d)->name : "<NONE!>",
          grn, nrm, yel, world_names[OLC_ZONE(d)->world-1],
          grn, nrm, cyn, buf2, 
  ...
  while (MYCMD.command != 'S') {
    ...
    case 'T':
      sprintf(buf2, "%sAttach trigger %s%s%s [%s%d%s] to %s",
                     MYCMD.if_flag ? " then " : "", cyn, 
                     trig_index[real_trigger(MYCMD.arg2)]->proto->name, yel,
                     cyn, MYCMD.arg2, yel, 
    ...
    case 'V':
      sprintf(buf2, "%sAssign global %s:%d to %s = %s", 
                      MYCMD.if_flag ? " then " : "", MYCMD.sarg1, MYCMD.arg2, 
                      ((MYCMD.arg1 == MOB_TRIGGER) ? "mobile" : 
                       ((MYCMD.arg1 == OBJ_TRIGGER) ? "object" : 
                        ((MYCMD.arg1 == WLD_TRIGGER)? "room" : "????"))),
                      MYCMD.sarg2);
...
void zedit_disp_flag_menu(struct descriptor_data *d) {
  ...
  for (counter = 0; counter < NUM_ZONE_FLAGS; counter++) {
    sprintf(buf, "%s%2d%s) %-20.20s %s", grn, counter+1, nrm,
      zone_flagbits[counter], !(++columns % 2) ?  "\r\n" : "");
    SEND_TO_Q(buf, d);
  }

  sprintbit(OLC_ZONE(d)->zflag, zone_flagbits, buf1);
  sprintf(buf, "\r\nRoom flags: %s%s%s\r\n"
 ...
 *  * The world names.
 *   */
void zedit_disp_world_menu(struct descriptor_data *d)
{
  int counter, columns = 0;
  ...
  for (counter = 0; counter < NUM_WORLDS; counter++) {
    sprintf(buf, "%s%2d%s) %-20.20s %s", grn, counter, nrm,
      world_names[counter], !(++columns % 2) ?  "\r\n" : "");
    SEND_TO_Q(buf, d);
  }

  SEND_TO_Q("\r\nEnter world number : ", d);
  OLC_MODE(d) = ZEDIT_WORLD;                                                    
}
...
/*-------------------------------------------------------------------*/
  case ZEDIT_WORLD:
    /*
     * Parse and add new world mode and return to main menu.
     */
    
    pos = atoi(arg);
    if (pos < 0 || pos > NUM_WORLDS-1) {
      zedit_disp_world_menu(d);
      return;
    }

    OLC_ZONE(d)->world = pos+1;
    OLC_ZONE(d)->number = 1;
    zedit_disp_menu(d);
    break;
  ...
  case ZEDIT_FLAGS:
    ...
    if (pos < 0 || pos > NUM_ZONE_FLAGS) {
      ...
      if (pos == 0) { 
        ...
        TOGGLE_BIT(OLC_ZONE(d)->zflag, 1 << (pos - 1)); 
	OLC_ZONE(d)->number = 1;
        zedit_disp_flag_menu(d); 
      }
    break;
