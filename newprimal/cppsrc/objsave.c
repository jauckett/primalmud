...
#define LOC_INVENTORY	-1 // DM - WEAR_LIGHT is 0!
...
{
  int j;
  dest->item_number = GET_OBJ_VNUM(obj);
#if USE_AUTOEQ
  dest->location = location;
#endif
  dest->value[0] = GET_OBJ_VAL(obj, 0);
  dest->value[1] = GET_OBJ_VAL(obj, 1);
  dest->value[2] = GET_OBJ_VAL(obj, 2);
  dest->value[3] = GET_OBJ_VAL(obj, 3);
  dest->extra_flags = GET_OBJ_EXTRA(obj);
  dest->weight = GET_OBJ_WEIGHT(obj);
  dest->timer = GET_OBJ_TIMER(obj);
  dest->bitvector = obj->obj_flags.bitvector;
  dest->damage = obj->damage;
  for (j = 0; j < MAX_OBJ_AFFECT; j++)
    dest->affected[j] = obj->affected[j];
}
...
int Obj_to_store(struct obj_data * obj, FILE * fl, int location)
{
  ...
  // NOTE - if anything is added here - also add to corpses.cpp obj saving

  Obj_to_file_elem(obj, &object, location);
...
void auto_equip(struct char_data *ch, struct obj_data *obj, int location)
{
  ...
  {	/* Was wearing it. */
	...
	if (CAN_WEAR(obj, ITEM_WEAR_HOLD))
	  break;
	if (IS_WARRIOR(ch) && CAN_WEAR(obj, ITEM_WEAR_WIELD) && 
	 ...
	 * zapped through the auto-equipping.
         */
         if (invalid_align(ch, obj) || invalid_class(ch, obj))
           location = LOC_INVENTORY;
         // DM, check level restrictions (eg. remort chars), could/should?
         // check rest of restrictions
         //else if (GET_OBJ_LEVEL(obj) > GET_LEVEL(ch))
         //  location = LOC_INVENTORY;
	 else if (invalid_level(ch, obj, FALSE))
	   location = LOC_INVENTORY;
         else
	 {
           equip_char(ch, obj, j, FALSE);
#ifndef NO_EXTRANEOUS_TRIGGERS
	   wear_otrigger(obj, ch, location);
#endif
	 }
...
int Crash_delete_file(char *name)
{
  ...
  if (!(fl = fopen(filename, "rb")))
  {
    if (errno != ENOENT)	/* if it fails but NOT because of no file */
      basic_mud_log("SYSERR: deleting crash file %s (1): %s",
  ...
  if (remove(filename) < 0 && errno != ENOENT)
    basic_mud_log("SYSERR: deleting crash file %s (2): %s",
	          filename, strerror(errno));
  return (1);
}
...
int Crash_delete_crashfile(struct char_data * ch)
{
  ...
  if (!(fl = fopen(fname, "rb")))
  {
    if (errno != ENOENT)	/* if it fails, NOT because of no file */
      basic_mud_log("SYSERR: checking for crash file %s (3): %s",
...
int Crash_clean_file(char *name)
{
  ...
  {
    if (errno != ENOENT)	/* if it fails, NOT because of no file */
      basic_mud_log("SYSERR: OPENING OBJECT FILE %s (4): %s",
	            fname, strerror(errno));
    return (0);
  }
  if (!feof(fl))
...
int Crash_load(struct char_data * ch)
{
  ...
  if (!(fl = fopen(fname, "r+b")))
  {
    if (errno != ENOENT)	/* if it fails, NOT because of no file */
    {
      basic_mud_log("SYSERR: READING OBJECT FILE %s (5): %s",
	            fname, strerror(errno));
      send_to_char("\r\n********************* NOTICE *********************\r\n"
  ...
  if (!feof(fl))
  {
    ...
    basic_mud_log("SYSERR: Crash_load: %s's rent file was empty!",
	          GET_NAME(ch));
    return (1);
  }

  if (!free_rent && 
      (rent.rentcode == RENT_RENTED || rent.rentcode == RENT_TIMEDOUT))
  {
    ...
    if (cost > GET_GOLD(ch) + GET_BANK_GOLD(ch))
    {
      ...
      sprintf(buf, "%s entering game, rented equipment lost (no $).",
	      GET_NAME(ch));
      mudlog(buf, BRF, MAX(LVL_IS_GOD, GET_INVIS_LEV(ch)), TRUE);
      Crash_crashsave(ch);
      return (2);
    } else {
  ...
  switch (orig_rent_code = rent.rentcode)
  {
    ...
    case RENT_TIMEDOUT:
      ...
      break;
    default:
      sprintf(buf, "SYSERR: %s entering game with undefined rent code %d.",
	      GET_NAME(ch), rent.rentcode);
      mudlog(buf, BRF, MAX(LVL_IS_GOD, GET_INVIS_LEV(ch)), TRUE);
      break;
  }
  ...
  while (!feof(fl))
  {
        ...
        if (cont_row[j])	/* No container, back to inventory. */
	{
          for (; cont_row[j]; cont_row[j] = obj2)
	  {
            ...
            obj_to_char(cont_row[j], ch, __FILE__, __LINE__);
          }
          cont_row[j] = NULL;
        }
      }
	...
	{ /* Remove object, fill it, equip again. */
          obj = unequip_char(ch, location - 1, FALSE);
          ...
          for (; cont_row[0]; cont_row[0] = obj2)
	  {
          ...
          equip_char(ch, obj, location - 1, FALSE);
        } else {		/* Object isn't container, empty the list. */
          for (; cont_row[0]; cont_row[0] = obj2)
	  {
            obj2 = cont_row[0]->next_content;
            obj_to_char(cont_row[0], ch, __FILE__, __LINE__);
          }
          cont_row[0] = NULL;
        }
      }
	...
	{ /* Take the item, fill it, and give it back. */
          ...
          for (; cont_row[j]; cont_row[j] = obj2)
	  {
          ...
          obj_to_char(obj, ch, __FILE__, __LINE__);	/* Add to inventory first. */
  ...
  // Artus> Reset their hit and value if they're above max.
  if ((GET_HIT(ch) > GET_MAX_HIT(ch)) &&
      !PRF_FLAGGED(ch, PRF_WOLF) &&
      !affected_by_spell(ch, SPELL_CHANGED))
...
void Crash_extract_norent_eq(struct char_data *ch)
{
  ...
  for (j = 0; j < NUM_WEARS; j++)
  {
    ...
    if (Crash_is_unrentable(GET_EQ(ch, j)))
      obj_to_char(unequip_char(ch, j, FALSE), ch, __FILE__, __LINE__);
    else
			 ...
			 long cost)
{
  ...
  sprintf(buf, "$n &rtells you, 'You can rent for &c%ld&| day%s with the gold you have\r\non hand and in the bank.'&n\r\n", rent_deadline, 
          (rent_deadline > 1) ? "s" : "");
			     ...
			     struct obj_data * obj)
{
  ...
  {
    has_norents = 1;
    sprintf(buf, "$n &rtells you, 'You cannot store &5%s&|.&n'", OBJS(obj, ch));
    act(buf, FALSE, recep, 0, ch, TO_VICT);
  }
		       ...
		       int display, int factor)
{
  ...
  if (!Crash_is_unrentable(obj))
  {
    ...
    *cost += MAX(0, (GET_OBJ_RENT(obj) * factor));
    if (display)
    {
      sprintf(buf, "$n &rtells you, '&Y%5d&| coins for &5%s&|..'&n",
		         ...
		         int display, int factor)
{
  ...
  if (!numitems)
  {
    act("$n &rtells you, 'But you are not carrying anything!  Just quit!'&n",
	FALSE, receptionist, 0, ch, TO_VICT);
    return (0);
  }
  if (numitems > max_obj_save)
  {
    sprintf(buf, "$n &rtells you, 'Sorry, but I cannot store more than &c%d&| items.'&n",
	    max_obj_save);
  ...
  if (display)
  {
    sprintf(buf, "$n &rtells you, 'Plus, my &Y%d&| coin fee..'&n",
	    min_rent_cost * factor);
    act(buf, FALSE, receptionist, 0, ch, TO_VICT);
    sprintf(buf, "$n &rtells you, 'For a total of &Y%ld&| coins%s.'&n",
	    totalcost, (factor == RENT_FACTOR ? " per day" : ""));
    act(buf, FALSE, receptionist, 0, ch, TO_VICT);
    if (totalcost > GET_GOLD(ch) + GET_BANK_GOLD(ch))
    {
      act("$n &rtells you, '...which I see you can't afford.'&n",
	  FALSE, receptionist, 0, ch, TO_VICT);
      return (0);
		         ...
		         int cmd, char *arg, int mode)
{
  ...
  if (free_rent)
  {
    act("$n &rtells you, 'Rent is free here.  Just quit, and your objects will be saved!'&n",
  ...
  if (CMD_IS("rent"))
  {
    if (!(cost = Crash_offer_rent(ch, recep, FALSE, mode)))
      return (TRUE);
    if (mode == RENT_FACTOR)
      sprintf(buf, "$n &rtells you, 'Rent will cost you &Y%d&| gold coins per day.'&n", cost);
    else if (mode == CRYO_FACTOR)
      sprintf(buf, "$n &rtells you, 'It will cost you &Y%d&| gold coins to be frozen.'&n", cost);
    act(buf, FALSE, recep, 0, ch, TO_VICT);
    if (cost > GET_GOLD(ch) + GET_BANK_GOLD(ch))
    {
      act("$n &rtells you, '...which I see you can't afford.'&n",
