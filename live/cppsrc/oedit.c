...
void oedit_setup_new(struct descriptor_data *d)
{
  ...
  OLC_BALBITS(d) = valid_obj(OLC_OBJ(d));
  oedit_disp_menu(d);
}
...
void oedit_save_internally(struct descriptor_data *d)
{
  ...
  if ((robj_num = add_object(OLC_OBJ(d), OLC_NUM(d), zone_table[OLC_ZNUM(d)].number)) < 0) {
...
void oedit_disp_battery_menu(struct descriptor_data *d)
{
  ...
  for (counter = 0; counter < NUM_BATTERY_TYPES; counter++) {
    sprintf(buf, "%s%2d%s) %s%-20.20s %s", grn, counter, nrm, yel,
		battery_types[counter], !(++columns % 3) ? "\r\n" : "");
    SEND_TO_Q(buf, d);
  }
  sprintf(buf, "\r\n%sEnter battery type : ", nrm);
  SEND_TO_Q(buf, d);
}
...
void oedit_disp_val1_menu(struct descriptor_data *d)
{
  ...
  case ITEM_POTION:
    ...
    break;
  case ITEM_TRAP:
    SEND_TO_Q("Loaded (0: No, 1: Yes, else: Yes, room) : ", d);
    break;
  case ITEM_WEAPON:
    /*
     * This doesn't seem to be used if I remembe right.
     * (was original to hitroll)
     *
     * 1st val for weapons is the spell number
     */
    SEND_TO_Q("\r\n(Spell will be cast if the MAGIC extra bit is set)\r\n", d);
  ...
  case ITEM_MAGIC_EQ:
    ...
    break;
  case ITEM_BATTERY:
    oedit_disp_battery_menu(d);
    break;
  case ITEM_JOINABLE:
    SEND_TO_Q("Vnum that obj joins with : ", d);
    break;
  case ITEM_GATEWAY:
    SEND_TO_Q("Room to teleport to : ", d);
    break;
  default:
    SEND_TO_Q("Value 0 (probably ignored): ", d);
    break;
    //oedit_disp_menu(d);
  }
}
...
void oedit_disp_val2_menu(struct descriptor_data *d)
{
  ...
  case ITEM_JOINABLE:
    ...
    break;
  case ITEM_BATTERY:
    SEND_TO_Q("Max storage value : ", d);
    break;
  case ITEM_GATEWAY:
    SEND_TO_Q("Minimum Level (0 = no restriction) : ", d);
    break;
  default:
    SEND_TO_Q("Value 1 (probably ignored): ", d);
    break;
    //oedit_disp_menu(d);
  }
}
...
void oedit_disp_val3_menu(struct descriptor_data *d)
{
  ...
  case ITEM_BATTERY:
    ...
    break;
  case ITEM_GATEWAY:
    SEND_TO_Q("Maximum Level (0 = no restriction) : ", d);
    break;
  default:
    SEND_TO_Q("Value 2 (probably ignored): ", d);
    break;
    //oedit_disp_menu(d);
  }
}
...
void oedit_disp_val4_menu(struct descriptor_data *d)
{
  ...
  case ITEM_STAFF:
    ...
    break;
  case ITEM_TRAP:
    SEND_TO_Q("Protecting item vnum : ", d);
    break;
  case ITEM_WEAPON:
  ...
  case ITEM_FOOD:
    ...
    break;
  case ITEM_MAGIC_EQ:
    SEND_TO_Q("Charge of item (-1 = unlimited) : ", d);
    break;
  case ITEM_JOINABLE:
    SEND_TO_Q("Vnum of obj that is created : ", d);
    break;
  case ITEM_BATTERY:
    SEND_TO_Q("Charge ratio (eq. 10 signifies 10:1) : ", d);
    break;
  case ITEM_GATEWAY:
    SEND_TO_Q("Cost to enter gateway: ", d);
    break;
  default:
    SEND_TO_Q("Value 3 (probably ignored): ", d);
    break;
    //oedit_disp_menu(d);
  }
}
...
void oedit_disp_menu(struct descriptor_data *d)
{
  ...
  sprintbit(OLC_BALBITS(d), obj_bits, unbalanced);
  ...
  sprintf(buf,
	  ...
	  grn, nrm, cyn, buf1,
	  //grn, nrm, cyn, ((GET_OBJ_TYPE(obj) == ITEM_FOUNTAIN || GET_OBJ_TYPE(obj) == ITEM_DRINKCON)) ? GET_OBJ_VAL(obj, 1) +5 : GET_OBJ_WEIGHT(obj),
	  ...
	  GET_OBJ_VAL(obj, 3),
	  grn, nrm, grn, nrm,
          grn, nrm, cyn, GET_OBJ_MAX_DAMAGE(obj),
  ...
  for (int i = 0; i < NUM_OBJ_ATTRIBUTES; i++) {
    if (IS_SET(OLC_BALBITS(d), (1 << i))) {
      sprintf(buf, "%s &g%s&n(&b%d&n)", buf, obj_bits[i], obj_attr(obj, (1 << i)));
    }
  }
  strcat(buf, "\r\nEnter choice : ");
...
void oedit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case OEDIT_CONFIRM_BALANCE:
    ...
    default:
      ...
      SEND_TO_Q("Are you sure you wish to save this object internally?\r\n", d);
      return;
    }
    break;
  ...
  case OEDIT_CONFIRM_SAVESTRING:
    ...
    case 'y':
    case 'Y':
      // DM - balance check before saving
      if (OLC_BALBITS(d) > 0) {
        sprintbit(OLC_BALBITS(d), obj_bits, buf);
        SEND_TO_Q("The following attributes break balance requirements:\r\n&g", d);
        SEND_TO_Q(buf, d);
        SEND_TO_Q("&n\r\nAre you sure you wish to save? : ", d);
	OLC_MODE(d) = OEDIT_CONFIRM_BALANCE;
	return;
      }
      SEND_TO_Q("Saving object to memory.\r\n", d);
      oedit_save_internally(d);
  ...
  case OEDIT_MAIN_MENU:
    ...
    case 'f':
    case 'F':
      SEND_TO_Q("Enter maximum object damage: ", d);
      OLC_MODE(d) = OEDIT_MAXDAMAGE;
      break;
    case 'm':
    case 'M':
  ...
  case OEDIT_LEVEL:
    //basic_mud_log("before change in obj level, obj_level = %d, arg = %d, balbits = %ld", GET_OBJ_LEVEL(OLC_OBJ(d)), atoi(arg), OLC_BALBITS(d));
    GET_OBJ_LEVEL(OLC_OBJ(d)) = atoi(arg);
    // Have changed the level - reaccess the balancing
    OLC_BALBITS(d) = valid_obj(OLC_OBJ(d));
    //basic_mud_log("after change in obj level, obj_level = %d, balbits = %ld", GET_OBJ_LEVEL(OLC_OBJ(d)), OLC_BALBITS(d));
    break;
  ...
  case OEDIT_MAXDAMAGE:
    GET_OBJ_MAX_DAMAGE(OLC_OBJ(d)) = atoi(arg);
    break;
