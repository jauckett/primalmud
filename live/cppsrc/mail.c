...
void store_mail(long to, long from, char *message_pointer)
{
  ...
  if (from < MAX_MAIL_FROM || to < 1 || !*message_pointer) {
    basic_mud_log("SYSERR: Mail system -- non-fatal error #5. (from == %ld, to == %ld)", from, to);
    return;
  }

  /* Quick hack to see if player is online and notify them of new mail */
  if (!(get_name_by_id(to))) // Artus> Sanity Check.
  {
    sprintf(buf, "SYSERR: Mail system -- Trying to store mail to non-exisant ID: %ld)", to);
    mudlog(buf, NRM, LVL_IMPL, TRUE);
    return;
  }

  // Artus> Replaced strcpy with strncpy.. Strcpy is teh bad.
  strncpy(toName, get_name_by_id(to), MAX_NAME_LENGTH);
  toName[0] = toupper(toName[0]);
  for (d = descriptor_list; d; d = d->next) {
    if ((d->character == NULL) || (STATE(d) != CON_PLAYING))
      continue;
    if (strcmp(GET_NAME(d->character), toName) == 0) {
      SEND_TO_Q("\007\007&gYou have new mail!&n\r\n", d);
      break;
    }      
  }
...
char *read_delete(long recipient)
{
  ...
  *(tmstr + strlen(tmstr) - 1) = '\0';
#else
  if (jk_localtime(&lt, header.header_data.mail_time))
  {
    basic_mud_log("Error in jk_localtime (mail_time: %ld) [%s:%d]", 
	header.header_data.mail_time, __FILE__, __LINE__);
    tmstr = NULL;
  } else {
    tmstr = asctime(&lt);
    *(tmstr + strlen(tmstr) - 1) = '\0';
  ...
  if (header.header_data.from < 1)
    from = mail_from_names[0-header.header_data.from];
  else
    from = get_name_by_id(header.header_data.from);

  to = get_name_by_id(recipient);
  sprintf(buf, " * * * * PrimalMUD Mail System * * * *\r\n"
	  "Date: &g%s&n\r\n"
	  "  To: &7%s&n\r\n"
	  "From: &7%s&n\r\n\r\n", ((tmstr == NULL) ? "Error" : tmstr), to ? to : "Unknown",
	  from ? from : "Unknown");
			  ...
			  int cmd, char *arg)
{
  ...
  if (LR_FAIL_MAX(ch, MIN_MAIL_LEVEL))
  {
    sprintf(buf, "$n &rtells you, 'Sorry, you have to be level %d to send mail!'&n", MIN_MAIL_LEVEL);
  ...
  if (!*buf) {			/* you'll get no argument from me! */
    act("$n &rtells you, 'You need to specify an addressee!'&n",
	FALSE, mailman, 0, ch, TO_VICT);
    return;
  }
  if ((GET_GOLD(ch) < STAMP_PRICE) && LR_FAIL(ch, LVL_IS_GOD))
  {
    sprintf(buf, "$n &rtells you, 'A stamp costs &Y%d&| coins.'&n\r\n"
	    "$n &rtells you, '...which I see you can't afford.'&n", STAMP_PRICE);
    act(buf, FALSE, mailman, 0, ch, TO_VICT);
    return;
  }
  /* GodMail */
  if( strcmp(buf, "gods") == 0) {
    recipient = get_id_by_name("Sandii");
    SET_BIT(PLR_FLAGS(ch), PLR_GODMAIL);
  } else
  if ((recipient = get_id_by_name(buf)) < 0) {
    act("$n&r tells you, 'No one by that name is registered here!'&n",
	FALSE, mailman, 0, ch, TO_VICT);
    return;
  }
  act("$n starts to write some mail.", TRUE, ch, 0, 0, TO_ROOM);
  sprintf(buf, "$n &rtells you, 'I'll take &Y%d&| coins for the stamp.'&n\r\n"
       "$n&r tells you, 'Write your message, "
       "(&c/s&| or &c@&| on a newline saves, &c/h&| for help).'&n", STAMP_PRICE);
			  ...
			  int cmd, char *arg)
{
  ...
  if (has_mail(GET_IDNUM(ch)))
    sprintf(buf, "$n &rtells you, 'You have mail waiting.'&n");
  else
    sprintf(buf, "$n &rtells you, 'Sorry, you don't have any mail waiting.'&n");
  act(buf, FALSE, mailman, 0, ch, TO_VICT);
}
