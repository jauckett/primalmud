...
void trigedit_save (struct descriptor_data *d, zone_vnum vznum);
void trigedit_create_index (int znum, char *type);
zone_rnum real_zone (zone_vnum vnum);

void
trigedit_string_cleanup (struct descriptor_data *d, int terminator)
{
  switch (OLC_MODE (d))
    {
...
void
script_copy (void *dst, void *src, int type)
{
  ...
  switch (type)
	...
	((struct room_data *) src)->proto_script;
      break;
    default:
      basic_mud_log ("SYSERR: Unknown type code sent to script_copy()!");
      break;
    }
...
/* be saved */
void
script_save_to_disk (FILE * fp, void *item, int type)
{
  ...
  if (type == MOB_TRIGGER)
    ...
    {
      basic_mud_log
	("SYSERR: Invalid type passed to script_save_mobobj_to_disk()");
      return;
    }
...
void
trigedit_setup_new (struct descriptor_data *d)
{
...
void
trigedit_setup_existing (struct descriptor_data *d, int rtrg_num)
{
  struct trig_data *trig;
...
void
trigedit_disp_menu (struct descriptor_data *d)
{
...
void
trigedit_disp_types (struct descriptor_data *d)
{
  int i, columns = 0;
  const char **types;
...
void
trigedit_parse (struct descriptor_data *d, char *arg)
{
  int i = 0;
  char *oldtext = NULL;
  ...
  switch (OLC_MODE (d))
	...
	case '6':
	  ...
	  send_editor_help (d);
	  SEND_TO_Q ("Enter trigger commands:\r\n\r\n", d);
	  if (OLC_STORAGE (d))
	    {
	      SEND_TO_Q (OLC_STORAGE (d), d);
	      oldtext = str_dup (OLC_STORAGE (d));
	    }
	  string_write (d, &OLC_STORAGE (d), MAX_TRIGGER_LENGTH, 0, oldtext);
/*
           send_to_char(
...
*/
	...
	{
	case 'y':
	  trigedit_save (d, zone_table[OLC_ZNUM (d)].number);
...
void
trigedit_save (struct descriptor_data *d, zone_vnum vznum)
{
  int trig_rnum, i;
  int found = 0;
  char *s;
  ...
  if ((trig_rnum = real_trigger (OLC_NUM (d))) != -1)
      ...
      if (!found)
	{
	  ...
	  new_index[trig_rnum]->vnum = OLC_NUM (d);
	  new_index[trig_rnum]->number = 0;
	  new_index[trig_rnum]->vznum = vznum;
	  new_index[trig_rnum]->rznum = OLC_ZNUM (d);
	  new_index[trig_rnum]->func = NULL;
	  ...
	  if (fprintf (trig_file, "#%d\n", i) < 0)
	    {
	      ...
	      mudlog (logbuf, BRF,
		      MAX (LVL_GOD, GET_INVIS_LEV (d->character)), TRUE);
	      fclose (trig_file);
	      return;
	    }
...
void
trigedit_create_index (int znum, char *type)
{
...
void
dg_olc_script_free (struct descriptor_data *d)
{
...
void
dg_olc_script_copy (struct descriptor_data *d)
{
...
void
dg_script_menu (struct descriptor_data *d)
{
...
int
dg_script_edit_parse (struct descriptor_data *d, char *arg)
{
