...
room_rnum add_room(struct room_data *room)
{
  ...
  for (i = top_of_world; i > 0; i--) {
    if (room->number > world[i - 1].number) {
      ...
      // DM - check this ... DOnt think it is needed
   ...
   */
  // DM - DEBUG INFO
  basic_mud_log("i = room->zone = %d, top_of_zone_table = %d", 
        room->zone, top_of_zone_table);
  for (i = room->zone; i <= top_of_zone_table; i++) {
    basic_mud_log("j = 0; ZCMD(%d(i), %d(j)).command = ", i, j);
  ...
  assign_triggers(&world[real_room(room->number)], WLD_TRIGGER);
  add_to_save_list(zone_table[room->zone].number, SL_WLD);
...
int save_rooms(zone_rnum rzone)
{
  ...
  basic_mud_log("GenOLC: save_rooms: Saving rooms in zone #%d (%d-%d).",
  ...
  for (i = zone_table[rzone].number * 100; i <= zone_table[rzone].top; i++) {
    ...
    if ((rnum = real_room(i)) != NOWHERE) {
      ...
      sprintascii(buf1, room->burgle_flags);
      fprintf(sf, 	"#%d\n"
			"%s%c\n"
			"%s%c\n"
			"%d %s %d %s\n",
		...
		zone_table[room->zone].number, buf2, room->sector_type, buf1
      );
...
int copy_room(struct room_data *to, struct room_data *from)
{
  ...
  *to = *from;
  ...
  // DM - check this ... Dont think it is needed ... ? 
  //to->proto_script = from->proto_script;
...
room_rnum duplicate_room(room_vnum dest_vnum, room_rnum orig)
{
  ...
  if (orig < 0 || orig > top_of_world) {
    basic_mud_log("SYSERR: GenOLC: copy_room: Given bad original real room.");
    return -1;
  } else if ((znum = real_zone_by_thing(dest_vnum)) < 0) {
    basic_mud_log("SYSERR: GenOLC: copy_room: No such destination zone.");
    return -1;
  }
  ...
  if ((new_rnum = add_room(&nroom)) < 0) {
    basic_mud_log("SYSERR: GenOLC: copy_room: Problem adding room.");
    return -1;
  }
