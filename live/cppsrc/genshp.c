...
void copy_shop(struct shop_data *tshop, struct shop_data *fshop)
{
  ...
  tshop->vznum = fshop->vznum;
  tshop->rznum = fshop->rznum;
...
int add_shop(struct shop_data *nshp)
{
  ...
  zone_vnum vznum = zone_table[rznum].number;
  ...
  for (rshop = top_shop - top_shop_offset; rshop > 0; rshop--) {
    if (nshp->vnum > SHOP_NUM(rshop - 1)) {
      ...
      shop_index[rshop].vznum = vznum;
      shop_index[rshop].rznum = rznum;
...
int save_shops(zone_rnum zone_num)
{
  ...
  if (zone_num < 1 || zone_num > top_of_zone_table) {
    basic_mud_log("SYSERR: GenOLC: save_mobiles: Invalid real zone number %d. (0-%d)", zone_num, top_of_zone_table);
    return FALSE;
  }
  ...
  // DM - recreate (reinsert the vzone number in the) index file 
  create_world_index(vzone, "shp");
