...
void parse_action(int command, char *string, struct descriptor_data *d)
{
  ...
  case PARSE_REPLACE:
    ...
    // Artus> Fix crash when /r'ing empty buffer.
    if (!d->str || !*d->str)
    {
  ...
  case PARSE_DELETE:
	...
	total_len--;
      *t = '\0';
      ...
      // DM - if we have an empty buffer, then free it 
      if (**(d->str) == '\0') {
        basic_mud_log("freeing empty buffer");
        free(*(d->str));
        *(d->str) = NULL;
        basic_mud_log("done freeing empty buffer");
      } else
