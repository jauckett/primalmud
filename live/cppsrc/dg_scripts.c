...
#include "db.h"
#include "screen.h"
#include "oasis.h" // Artus> GET_OLC_ZONE
...
{
  /* // Artus>> MOVED to mobile_activity() 
  char_data *ch;
  obj_data *obj;
  ...
  */
  ...
  /* // Artus> Moved to object_activity()
  for (obj = object_list; obj; obj = obj->next)
  {
    if (SCRIPT(obj))
    {
...
ACMD(do_attach) 
{
  ...
  if (!*arg || !*targ_name || !*trig_name) {
    send_to_char("Usage: attach mtr {trig vnum} {mob alias} [location] (mobs)\r\n       attach wtr {trig vnum} {room vnum} [location] (rooms)\r\n       attach otr {trig vnum} {obj alias} [location] (objects)\r\n", ch);
    return;
  }
  
  if ((GET_OLC_ZONE(ch) < 1) && LR_FAIL(ch, LVL_IMPL))
  {
    send_to_char("You do not have a zone assigned.\r\n", ch);
    return;
  }
  ...
  {
    if ((victim = generic_find_char(ch, targ_name, FIND_CHAR_ROOM |
	                                           FIND_CHAR_WORLD)))
    {
      if (IS_NPC(victim))
      {
       /* Artus> <IMPL can only attach to their own zone */
       if ((GET_MOB_VZNUM(victim) != GET_OLC_ZONE(ch)) && LR_FAIL(ch, LVL_IMPL))
       {
    ...
    if ((object = generic_find_obj(ch, targ_name, FIND_OBJ_EQUIP | FIND_OBJ_WORLD | FIND_OBJ_ROOM | FIND_OBJ_INV)))
    {
      /* Artus> <IMPL can only attach to their own zone */
      if ((GET_OBJ_VZNUM(object) != GET_OLC_ZONE(ch)) && LR_FAIL(ch, LVL_IMPL))
      {
        send_to_char("You do not have permission to edit this zone.\r\n", ch);
        return;
      }
    ...
    if (isdigit(*targ_name) && !strchr(targ_name, '.')) 
    {
      ...
      {
        /* Artus> <IMPL can only attach to their own zone */
        if ((zone_table[world[room].zone].number != GET_OLC_ZONE(ch)) &&
	    LR_FAIL(ch, LVL_IMPL))
        {
          send_to_char("You do not have permission to edit this zone.\r\n", ch);
          return;
...
void add_var(struct trig_var_data **var_list, char *name, char *value, long id)
{
  ...
  //basic_mud_log("Loggin variable: %s, value = %s, id = %ld", name, value, id);
...
{  
  ...
  if (!str_cmp(arg1, "room"))
  {
    ...
    {
      if (!(victim = generic_find_char(ch, arg2, FIND_CHAR_ROOM | 
	                                         FIND_CHAR_WORLD)))
      ...
      if (!(object = generic_find_obj(ch, arg2, FIND_OBJ_INV | FIND_OBJ_ROOM |
	                                        FIND_OBJ_WORLD)))
      ...
      if (generic_find(arg1, FIND_OBJ_ROOM | FIND_OBJ_INV | FIND_CHAR_WORLD | FIND_CHAR_ROOM | FIND_OBJ_WORLD, ch, &victim, &object) < 1)
      {
    ...
    if (victim)
    {
      ...
      /* Artus> <IMPL can only attach to their own zone */
      else if ((GET_MOB_VZNUM(victim) != GET_OLC_ZONE(ch)) && 
	       LR_FAIL(ch, LVL_IMPL))
        send_to_char("You do not have permission to edit this zone.\r\n", ch);
                ...
                int type, char *var, char *field, char *subfield, char *str)
{
  ...
  char *zoneecho_cmd[] =   {"mzoneecho",   "ozoneecho",   "wzoneecho"};
  ...
  if (!*field) {
	...
	strcpy(str,zoneecho_cmd[type]);
      else
       *str = '\0';

      //basic_mud_log("TEST: str = '%s'\r\n", str);
    }
    ...
    if (vd) {
      ...
      else if (!str_cmp(var, "firstchar")) {
	if (!(*field) || !is_number(field) || ((num = real_room(atoi(field))) == NOWHERE)) {
	  sprintf(buf2, "Trigger: %s, VNum %d. invalid 'firstchar' room: '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), subfield);
	  script_log(buf2);
	} else {
	  if (world[num].people)
	    sprintf(str,"%c%ld", UID_CHAR, GET_ID(world[num].people));
	  else 
	    *str = '\0';
	  return;
	}
...
#endif
      else if (!str_cmp(field, "first_in_room"))
	sprintf(str, "%c%ld", UID_CHAR, GET_ID(world[IN_ROOM(c)].people));
      ...
      } else if (!str_cmp(field, "affected")) {
	if (!(*subfield))
	  sprintf(str, "%ld", AFF_FLAGS(c));
	else if (is_number(subfield))
	{
	  int an = atoi(subfield);
	  if ((an < 0) || (an > 31))
	  {
	    strcpy(str, "0");
            sprintf(buf2, 
                   "Trigger: %s, VNum %d. affected subfield out of range: '%s'",
                   GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), subfield);
            script_log(buf2);
	  } else {
	    if AFF_FLAGGED(c, (1 << an))
	      strcpy(str, "1");
	    else
	      strcpy(str, "0");
	  }
	  ...
	  strcpy(str, "-1");
	  for (int an = 0; an < MAX_AFFECT; an++)
	    if (!str_cmp(subfield, affected_bits[an]))
	    {
	      if AFF_FLAGGED(c, (1 << an))
		strcpy(str, "1");
	      else
	  ...
	  if (str[0] == '-')
	  {
	    strcpy(str, "0");
            sprintf(buf2, 
                   "Trigger: %s, VNum %d. affected subfield not found: '%s'",
                   GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), subfield);
            script_log(buf2);
	  }
	}
        ...
        if (c->next_in_room)
	{
	  if (!(*subfield))
	  {
	  ...
	  } else if (!str_cmp("vnum", subfield)) {
	    if (!IS_NPC(c->next_in_room))
	      strcpy(str, "-1");
	    else
	      sprintf(str, "%d", GET_MOB_VNUM(c->next_in_room));
	  } else if (!str_cmp("name", subfield)) {
	    sprintf(str, "%s", GET_NAME(c->next_in_room));
	  } else if (!str_cmp("can_see", subfield)) {
	    if (CAN_SEE(c, c->next_in_room))
	      strcpy(str, "1");
	    else
	      strcpy(str, "");
	  } else if (!str_cmp("can_see_me", subfield)) {
	    if (CAN_SEE(c->next_in_room, c))
	      strcpy(str, "1");
	    else
	      strcpy(str, "");
	  } else {
	    ...
	    strcpy(str, "");
            sprintf(buf2, 
                   "Trigger: %s, VNum %d. unknown next_in_room subfield: '%s'",
                   GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), subfield);
            script_log(buf2);
	  }
	} else strcpy(str,"");
	...
	return;
      if (!str_cmp(field, "id")) {
	sprintf(str, "%d", real_room(r->number) + 50000); 
      } else if (!str_cmp(field, "name")) {
       strcpy(str, r->name);
        ...
        if (r->dir_option[NORTH])
	{
	  if (!*subfield)
	    ...
	    sprintf(str, "%d", real_room(r->number) + 50000); 
	  else if (!str_cmp(subfield, "vnum"))
	    sprintf(str, "%d", world[r->dir_option[NORTH]->to_room].number);
	  else {
	    *str = '\0';
	    sprintf(buf2, "Trigger: %s, VNum %d, type: %d, unknown %s field: '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), type, field, subfield);
	    script_log(buf2);
	  }
	}
        else
        ...
        if (r->dir_option[EAST])
	{
	  if (!*subfield)
	    ...
	    sprintf(str, "%d", world[r->dir_option[EAST]->to_room].number);
	  else {
	    *str = '\0';
	    sprintf(buf2, "Trigger: %s, VNum %d, type: %d, unknown %s field: '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), type, field, subfield);
	    script_log(buf2);
        ...
        if (r->dir_option[SOUTH])
	{
	  if (!*subfield)
	    ...
	    sprintf(str, "%d", world[r->dir_option[SOUTH]->to_room].number);
	  else {
	    *str = '\0';
	    sprintf(buf2, "Trigger: %s, VNum %d, type: %d, unknown %s field: '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), type, field, subfield);
	    script_log(buf2);
        ...
        if (r->dir_option[WEST])
	{
	  if (!*subfield)
	    ...
	    sprintf(str, "%d", world[r->dir_option[WEST]->to_room].number);
	  else {
	    *str = '\0';
	    sprintf(buf2, "Trigger: %s, VNum %d, type: %d, unknown %s field: '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), type, field, subfield);
	    script_log(buf2);
        ...
        if (r->dir_option[UP])
	{
	  if (!*subfield)
	    ...
	    sprintf(str, "%d", world[r->dir_option[UP]->to_room].number);
	  else {
	    *str = '\0';
	    sprintf(buf2, "Trigger: %s, VNum %d, type: %d, unknown %s field: '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), type, field, subfield);
	    script_log(buf2);
        ...
        if (r->dir_option[DOWN])
	{
	  if (!*subfield)
	    ...
	    sprintf(str, "%d", world[r->dir_option[DOWN]->to_room].number);
	  else {
	    *str = '\0';
	    sprintf(buf2, "Trigger: %s, VNum %d, type: %d, unknown %s field: '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), type, field, subfield);
	    script_log(buf2);
	...
	if (!(*subfield))
	{
	  if (r->people)
	    ...
	    *str = '\0';
	} else if (!str_cmp(subfield, "countpc")) {
	  for (k = r->people; k; k = k->next_in_room)
	    if (!IS_NPC(k)) 
	      pcount++;
	  sprintf(str, "%d", pcount);
	} else if (!str_cmp(subfield, "countnpc")) {
	  for (k = r->people; k; k = k->next_in_room)
	    if (IS_NPC(k)) 
	      pcount++;
	  sprintf(str, "%d", pcount);
	} else if (!str_cmp(subfield, "countall")) {
	  for (k = r->people; k; k = k->next_in_room)
	    pcount++;
	  sprintf(str, "%d", pcount);
	} else {
	  *str = '\0';
	  sprintf(buf2, "Trigger: %s, VNum %d, type: %d, unknown people subfield: '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), type, subfield);
	  script_log(buf2);
	}
      } else {
       *str = '\0';
