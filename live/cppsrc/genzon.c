...
extern struct index_data **trig_index; /* index table for triggers      */
...
void create_world_index(int znum, const char *type)
{
  ...
  // DM - TODO FIX - if zone already exists, don't insert it again...
...
int save_zone(zone_rnum zone_num)
{
   ...
   */
  fprintf(zfile, "#%d %d\n" "%s~\n" "%d %d %d %d\n",
	  zone_table[zone_num].number, zone_table[zone_num].world,
  ...
  for (subcmd = 0; ZCMD(zone_num, subcmd).command != 'S'; subcmd++) {
    ...
    case 'T':  
      ...
      arg2 = ZCMD(zone_num, subcmd).arg2;
      comment = trig_index[real_trigger(ZCMD(zone_num, subcmd).arg2)]->proto->name; 
      break;   
    case 'V':  
      arg1 = ZCMD(zone_num, subcmd).arg1; /* trigger type */
      arg2 = ZCMD(zone_num, subcmd).arg2; /* context */
      comment = "Global variable assignment";
      break;
    case '*':
      /*
    ...
    if (ZCMD(zone_num, subcmd).command != 'V') { 
      ...
      fprintf(zfile, "%c %d %d %d %d %s %s\n",
                ZCMD(zone_num, subcmd).command, ZCMD(zone_num, subcmd).if_flag,
                arg1, arg2, arg3, ZCMD(zone_num, subcmd).sarg1, 
