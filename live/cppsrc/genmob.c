...
int add_mobile(struct char_data *mob, mob_vnum vnum, zone_vnum vznum)
{
  ...
  top_of_mobt++;
  zone_table[real_zone(vznum)].nomobs++;
...
int write_mobile_record(mob_vnum mvnum, struct char_data *mob, FILE *fd)
{
  ...
  if (write_mobile_mobprog(mvnum, mob, fd) < 0)
    basic_mud_log("SYSERR: GenOLC: Error writing MobProgs for mobile #%d.", mvnum);
#endif
